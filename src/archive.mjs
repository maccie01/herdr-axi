...
import { stateRoot, hash, DEFAULT_CONFIG } from "./project.mjs";
...
export function finishRun() {
  ...
  const result = changeRun((r) => {
    ...
    fs.writeFileSync(archive + ".tmp", gzipSync(JSON.stringify(r)), { mode: 0o600 });
  ...
  for (const w of r.workers) {
    ...
    files.push(expected, expected + ".inbox", path.join(path.dirname(expected), w.name + ".json"));
