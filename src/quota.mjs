...
export function quotaError(text) {
  ...
  for (let i = lines.length - 1; i >= 0; i--) {
    ...
    const match = line.match(/^(?:[✗×!]\s*)?(You have exceeded your monthly quota|Session limit reached|You(?:'|’)ve hit your (?:usage|session) limit)(?:\s|[.!:(]|$)/i);
    if (match) return { code: "QUOTA_EXHAUSTED", scope: /monthly/i.test(match[1]) ? "monthly" : "session", message: line.slice(0, 240) };
  }
  return null;
}
