...
## Final validation and branch split — 7 September 2026

| Check | Result |
| --- | --- |
| Full Node suite | 187/187 passed; no failures or skips |
| Full Bash monitor suite | 64/64 passed; isolated fake backend |
| Additional CLI checks | Operator transcripts; before/after reproduction of report diagnostics and monitor fixtures |
| Real recovery rounds | Six rounds; 11 owned workers closed; no foreign pane input; all six runs archived |
| Independent review | Assignment fencing, monitor identity and final diagnostic/fixture fixes checked |
| no-mistakes | v1.64.0; test, document and lint completed; run `01M1XRBHEA8NPV90E8HZHC5ZN1` |
| Review limitation | Two automated fix/re-review rounds timed out; final automated re-review skipped, not passed |
| CI / publication | CI skipped; no npm publication; Git push handled separately |
| Branch policy | Consumer `main`; this evidence and figure source notes retained on `dev`; identical runtime/tests |

- Earlier counts below: historical checkpoints, not the final integrated result.
- Runtime transcripts, state and receipts remain outside the repository; this file retains curated evidence only.
