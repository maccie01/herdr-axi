...
const self = fileURLToPath(new URL("./fixtures/basic-herdr.mjs", import.meta.url));
const cli = fileURLToPath(new URL("../bin/herdr-axi.mjs", import.meta.url));
const pane = "w1:pTEST";

test("standalone fake backend rejects missing and unknown commands without running tests", () => {
  for (const args of [[], ["unsupported-command"]]) {
    const result = spawnSync(process.execPath, [self, ...args], {
      encoding: "utf8", timeout: 2000,
    });
    assert.ifError(result.error);
    assert.equal(result.status, 2);
    assert.match(result.stderr, /unhandled command/);
...
test("delegation help is step-specific, bounded and never contacts Herdr", () => {
  ...
  for (const action of ["init", "config", "queue", "move", "switch", "next", "status", "inbox", "accept", "revise", "phase", "close", "cancel", "recover", "leases", "unlock", "finish", "history", "gc"]) {
